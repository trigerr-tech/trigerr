...
from . import AnalysisIndicators, Category
...
def bind(name: str, fn: types.FunctionType, method: types.MethodType = None):
    ...
    setattr(AnalysisIndicators, name, method)
...
def import_dir(path: str, verbose: bool = True):
    ...
    for d in dirs:
        ...
        # only look in directories which are valid ta categories
        if dirname not in [*Category]:
            if verbose and dirname not in ["__pycache__", "__init__.py"]:
                print(
                    f"[i] Skipping the sub-directory '{dirname}' since it's not a valid ta category."
                )
            continue
        ...
        for module in glob(abspath(join(path, dirname, "*.py"))):
            ...
            if module_name not in ["__init__"]:
                ...
                if module_name not in Category[dirname]:
                    Category[dirname].append(module_name)
